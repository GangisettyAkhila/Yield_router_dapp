...
This folder contains the React + TypeScript frontend for the Yield Router Gaming DApp.

Product overview, goals and core features:

- Users stake ALGOs into a Yield Router which allocates liquidity across DeFi platforms and mints GameCredits/StakeCredits from yield.
- Users can play games (1v1 cricket initially) using GameCredits, or spectate and stake ALGOs on match outcomes.
- Smart contracts are in AlgoPy and the frontend wraps generated ARC-56 clients (algokit). The UI uses @txnlab/use-wallet-react for wallet integration.

Files of interest

- `src/contracts/YieldRouterContract.ts` - autogenerated ARC-56 client.
- `src/contracts/yieldRouterHelpers.ts` - small helper wrappers to read user tracking and claim yield.
- `src/contracts/GameMatchContract.ts` - lightweight typed wrapper used by UI components to create/join/submit matches.

Next steps for developers

- Wire `yieldRouterHelpers` into UI components like `StakeForm`, `StakeOnMatchForm`, and `MatchList`.
- Generate full Algokit clients for other contracts (GameMatch, StakeMarket, Leaderboard) when ABI specs are available.
